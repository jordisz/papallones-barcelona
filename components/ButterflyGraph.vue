...
export default {
  ...
  methods: {
    drawGraph() {
      ...
      const svg = canvas.append("svg").attr("width", 300).attr("height", 100);
      const xScale = d3.scaleLinear().domain([9, 44]).range([25, 275]);
      const rScale = d3.scaleLinear().domain([0, 6000]).range([2, 210]);
      const tickLabels = [
        "Mar",
        "Abr",
        "Mai",
      ...
      svg
        ...
        .attr("fill", "#4747D1");
      svg.append("g").call(xAxis).attr("transform", "translate(0,68)");
    },
    clearGraph() {
      const canvas = d3.select(`.id${this.id}`);
