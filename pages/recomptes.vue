<template>
  <div>
    ...
    <div v-if="isDataReady" class="page-container">
      <h1 class="titol">Recomptes als parcs de Barcelona</h1>
      <div class="summary">
        <div>
          {{ parcName }} ({{ selectedYear }}) -
          {{ countOnlyDeterminedSpecies }} espècies
        </div>
      </div>
      <div class="filters-bar">
        <AppSelector
          class="selector"
          id-name="filtraParc"
          label="Filtra per parc"
          :list="parcNamesArray"
          @item-selected="filtraParc"
        />
        <AppSelector
          class="selector"
          id-name="filtraAny"
        ...
        <AppRadioButtonGroup
          id="ordre"
          v-model="orderCriterion"
          :checked="orderCriteria[0]"
          :options="orderCriteria"
        >
          Ordena per:
        </AppRadioButtonGroup>
      </div>
      <transition-group name="flip-cards" tag="div" class="cards-container">
        <ButterflyCard
          v-for="especie in filteredByYear"
          :key="especie[0]"
          :especie="especie"
        />
      </transition-group>
  ...
  computed: {
    ...
    filteredByYear() {
      ...
      this.byParcData.forEach((especie) => {
        ...
        tempEspecie[1] = especie[1].filter((d) =>
          d.fecha.includes(this.selectedYear)
        );
    ...
    countOnlyDeterminedSpecies() {
      const undetermined = this.EspeciesInfo.filter(
        (especie) => especie.determinada === false
      ).map((x) => x.nomCientific);
      return this.filteredByYear.filter(
        (especie) => !undetermined.includes(especie[0])
      ).length;
    },
  },
  watch: {
